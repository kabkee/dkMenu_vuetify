...
const onClickStart = () => {
    ...
    loading.value = true;
    memberCount.value = memberCount.value;
    setTimeout(() => {
        init();
    }, 310)
}
...
function canvasDraw() {
    ladder.value.css({
        'width': (memberCount.value - 1) * 100 + 6,
        'height': (heightNode - 1) * 25 + 6,
        'background-color': '#fff'
    });
    ladder_canvas
        .attr('width', (memberCount.value - 1) * 100 + 6)
...
function drawNodeLine() {
    ...
    for (var y = 0; y < heightNode; y++) {
        for (var x = 0; x < memberCount.value; x++) {
            var node = x + '-' + y;
            var nodeInfo = GLOBAL_FOOT_PRINT[node];
...
function drawDefaultLine() {
    ...
    for (var y = 0; y < heightNode - 1; y++) {
        html += '<tr>';
        for (var x = 0; x < memberCount.value - 1; x++) {
...
function setDefaultFootPrint() {
    ...
    for (var r = 0; r < heightNode; r++) {
        for (var column = 0; column < memberCount.value; column++) {
...
<template>
    <div class="w-full h-full items-center justify-center" v-show="loading">
        <div class="w-full h-full flex flex-col items-center justify-center text-center p-4">
            <div class='w-[400px] h-[200px] bg-no-repeat bg-center'
                :style="{ 'background-image': `url(assets/ladder_bg.png)` }"></div>
            <div class="landing-form">
                <div class="group">
                    <v-text-field type='number' clearable label="참여자 수" v-model="memberCount" />

                    <span class="highlight"></span>
                    <span class="bar"></span>
                    <v-btn color="success" class="px-10" @click="onClickStart">
                        START
                    </v-btn>
                </div>
            </div>
    ...
    <div ref="ladder">
        <div class="dim"></div>
        <canvas ref="canvas" id="ladder_canvas"></canvas>
    </div>
</template>
